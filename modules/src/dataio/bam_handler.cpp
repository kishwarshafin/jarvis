...
int count_total_matches(int n_cigar, const uint32_t *cigar)
{
    int k;
    int matches = 0;
    for (k = 0; k < n_cigar; ++k)
        if (bam_cigar_op(cigar[k]) == BAM_CMATCH || bam_cigar_op(cigar[k]) == BAM_CDIFF || bam_cigar_op(cigar[k]) == BAM_CEQUAL)
            matches += bam_cigar_oplen(cigar[k]);
    return matches;
}
...
vector <type_read> BAM_handler::get_reads(string chromosome,
    ...
    while(sam_itr_next(this->hts_file, iter, alignment) >= 0) {
        ...
        int aligned_length = bam_cigar2rlen(alignment->core.n_cigar, cigar);
        int total_matches = count_total_matches(alignment->core.n_cigar, cigar);
...
//                    }
        ...
        read.hp_tag = HP_tag;
        read.len = len;
        read.aligned_len = aligned_length;
        read.total_matches = total_matches;
